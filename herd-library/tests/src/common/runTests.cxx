...
#define BOOST_TEST_MODULE HerdTest
...
{
  const char* lLogLevelVar = getenv("HERD_TEST_LOG_LEVEL");
  if (lLogLevelVar != NULL) {
    ...
    if (lIt != lLogLevelMap.end())
      ...
      std::cout << "Invalid value '" << lLogLevelVar << "' given for HERD_TEST_LOG_LEVEL env var. Valid values: " << std::endl;
