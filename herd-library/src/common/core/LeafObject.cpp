  ...
  mParent(0x0)
{
  ...
  for (size_t i = 0; i < mId.size(); i++) {
    if ( (isalnum(mId[i]) == 0) and (mId[i] != '_') )
      throw InvalidObjectId(std::string("Found invalid character '") + mId[i] + "' at position " + boost::lexical_cast<std::string>(i) + " in ID '" + mId + "'; ID strings must only contain alphanumeric characters." );
  }
}
...
void LeafObject::setParent(const LeafObject* aParent)
{
  if (mParent) {
    ...
    throw FailedToSetParent(lMsgStream.str());
  }
