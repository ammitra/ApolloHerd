...
void Object::addObj(LeafObject* aChild)
{
  ...
  if (mObjectsChart.find(aChild->getId()) != mObjectsChart.end()) {
    ...
    delete aChild;
    throw ObjectOfSameIdAlreadyExists(lErrorMsg);
  }
  ...
  if ( (not aChild->getAlias().empty()) and (mChildAliases.find(aChild->getAlias()) != mChildAliases.end()) ) {
    ...
    delete aChild;
    throw ObjectOfSameAliasAlreadyExists(lErrorMsg);
  }
...
void Object::getCharts(const std::string& aBasePath, std::unordered_map<std::string, LeafObject*>& aChart) const
{
  std::string lPrefix(aBasePath.empty() ? "" : aBasePath + '.');
  // 1) Add children
  std::unordered_map<std::string, LeafObject*>::const_iterator lItMap;
...
std::vector<std::string> Object::getChildren() const
{
  ...
  for (std::unordered_map<std::string, LeafObject*>::const_iterator lIt = mObjectsChart.begin(); lIt != mObjectsChart.end(); lIt++) {
...
{
  std::unordered_map<std::string, LeafObject*> lChart;
  this->getCharts("", lChart);
  std::vector<std::string> lNames;
  lNames.reserve(lChart.size());
  std::unordered_map<std::string, LeafObject*>::const_iterator lIt;
...
LeafObject& Object::getObj(const std::string& aId)
{
  ...
  if ((lIt = mObjectsChart.find(lFather)) == mObjectsChart.end()) {
    ...
    throw ObjectDoesNotExist(lSS.str());
  }
  ...
  if (lChild.empty()) {
    ...
    throw ObjectDoesNotExist(lMsgStream.str());
  }
}
...
size_t Object::getNumberOfGenerationsTo(const LeafObject& aDescendant) const
{
  ...
  throw ObjectIsNotDescendant("Object '" + aDescendant.getPath() + "' is not a descendant of '" + getPath() + "'");
}
...
void ObjectView::addObj(LeafObject* aChild, const std::string& aAlias)
{
  // Insure the child does not have a twin
  if (mObjectsChart.find(aChild->getId()) != mObjectsChart.end()) {
    throw ObjectOfSameIdAlreadyExists("Object with ID '" + aChild->getId() + "' already exists as child of '" + getPath() + "'");
  }

  if ( not aChild->getAlias().empty() and (mChildAliases.find(aChild->getAlias()) != mChildAliases.end()) ) {
    throw ObjectOfSameAliasAlreadyExists("Object with alias '" + aChild->getAlias() + "' already exists as child of '" + getPath() + "'");
  }  
