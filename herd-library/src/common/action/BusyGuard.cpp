...
//------------------------------------------------------------------------------------
BusyGuard::BusyGuard(Functionoid& aAction, ActionableStatus& aStatus, const BusyGuard* aOuterGuard) :
...
//------------------------------------------------------------------------------------
BusyGuard::BusyGuard(Functionoid& aAction, ActionableStatus& aStatus, ActionableStatusGuard& aStatusGuard, const Callback_t& aCallback, const BusyGuard* aOuterGuard) :
  mActionableObj(aAction.getActionable()),
  mStatus(aStatus),
  mAction(aAction),
  mOuterGuard(aOuterGuard),
...
BusyGuard::BusyGuard(ActionableObject& aResource, ActionableStatus& aStatus, ActionableStatusGuard& aStatusGuard, const Functionoid& aAction, const Adopt) :
  ...
  mOuterGuard(NULL)
{
  if ( mStatus.getRunningActions(aStatusGuard).size() != size_t(1) )
    ...
    throw WrongBusyGuard("BusyGuard cannot adopt action '"+mAction.getPath()+"' on object '"+mActionableObj.getPath()+"' : Object is running "+ActionFmt(mStatus.getRunningActions(aStatusGuard).at(0)).str());
