...
//  Object* getAncestor ( const uint32_t& aDepth = 1 );
  ...
  */
  void getCharts ( const std::string& aBasePath, std::unordered_map<std::string, LeafObject*>& aChart ) const;
  ...
  //! Map of children
  std::unordered_map< std::string, LeafObject* > mObjectsChart;

  std::unordered_set<std::string> mChildAliases;
...
T& Object::getObjByFullPath(const std::string& aFullIdPath)
{
  if (aFullIdPath.find(getPath()) != 0)
    throw ObjectDoesNotExist( "Beginning of full object ID path '" + aFullIdPath + "' does not match path of object '" + getPath() + "'");
  else
