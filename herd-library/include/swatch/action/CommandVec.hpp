...
public:
  ...
  void checkForMissingParameters(const GateKeeper& aGateKeeper, std::vector<core::ParameterSet>& aParamSets, std::vector<MissingParam>& aMissingParams) const;

  //! Data structure for the rule violations of a single command
  struct ParamRuleViolationList {
    ParamRuleViolationList(const std::string& aCommand, const size_t aCommandIdx, const core::ParameterSet& aParamSet, const std::vector<Command::ParamRuleViolation>& aRuleViolation);
    std::string command;
    size_t commandIdx;
    core::ParameterSet paramSet;
    std::vector<Command::ParamRuleViolation> violations;
  };
  
  void checkForInvalidParameters(const std::vector<core::ParameterSet>& aParamSets, std::vector<ParamRuleViolationList>& aRuleViolations) const;

protected:
  typedef std::vector<core::ParameterSet> ParameterSets_t;
   ...
   */
  void extractParameters(const GateKeeper& aGateKeeper, std::vector<core::ParameterSet>& aParamSets, std::vector<MissingParam>& aMissingParams, bool aThrowOnMissing) const;

  //! thread safe exception-catching wrapper for code()
  void runCommands(std::shared_ptr<BusyGuard> aGuard);

  template<class OBJECT>
  void scheduleAction( OBJECT* cmd , boost::function<void(OBJECT*, std::shared_ptr<BusyGuard>)> aFunction, const std::shared_ptr<BusyGuard>& resourceGuard );
...
template<class OBJECT>
void CommandVec::scheduleAction( OBJECT* aAction , boost::function<void(OBJECT*, std::shared_ptr<BusyGuard>)> aFunction, const std::shared_ptr<BusyGuard>& aGuard )
{
...
public:
  ...
  const std::vector<const boost::any*>& getResults() const;
...
private:
  ...
  std::vector<const boost::any*> mResults;
