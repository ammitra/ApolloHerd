...
public:
  ...
  void exec(const std::vector<core::ParameterSet>& aParamSets, const bool& aUseThreadPool = true );

  void exec(const BusyGuard* aGuard, const std::vector<core::ParameterSet>& aParamSets, const bool& aUseThreadPool = true );
...
private:
  void run(std::shared_ptr<BusyGuard> aGuard);
...
public:
   ...
   */
  void engage(const ActionableStatusGuard& aGuard);
   ...
   * @param aGuard Guard that's locking the status object of the associated actionable object
   */
  void reset(const ActionableStatusGuard& aGuard);
